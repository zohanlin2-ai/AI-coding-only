...
graph TD
    ...
    M -->|背景執行自我檢測 --self-test| O{檢測是否通過？}
    O -->|否| P[執行回滾: 還原 versions/ 至 current/]
    P --> N1[中止更新並重啟舊版]
    O -->|是| Q[寫入本地版本檔]
    Q --> N[完成更新]
    N -->|重新拉起子進程| C
...
### 四、 更新後自我檢測與回滾機制 (Post-Update Self-Test & Rollback)

為了確保更新後應用程式能夠正常啟動，本架構引入了雙重防禦機制：

#### 1. 靜態自我檢測 (`--self-test` 參數)
在更新器 (`updater.py`) 將新代碼覆蓋至 `/current` 目錄後，會立即在背景執行：
```bash
python current/assistant.py --self-test [--cli]
```
* **工作邏輯**：主程式會在載入 UI 前優先解析此參數，僅執行核心套件導入、配置檔載入等基本初始化。
* **判定機制**：若順利載入且無拋出錯誤，程式會以 `exit(0)` 退出；若發生 `ImportError` 或配置解析錯誤，則會以 `exit(1)` 退出。
* **回滾觸發**：若自我檢測回傳非 0，`updater.py` 會立刻清除當前損壞的 `/current` 資料夾，並將 `versions/v[舊版本]` 還原回去，確保系統不崩潰。

#### 2. 動態啟動期監控 (10秒觀察期)
若靜態檢測通過，但主程式在完全拉起後因特定執行期錯誤快速崩潰：
* **時間臨界點**：`launcher.py` 會監控新程式啟動後的運作時間。
* **判定機制**：若主程式在啟動後 **10 秒內** 崩潰退出（結束碼 $\neq 0$ 且 $\neq 42$），啟動器即判定為新版執行期錯誤，會呼叫更新模組的 rollback 方法，將主程式還原為前一個穩定版本並重新拉起。
* **防無窮迴圈**：執行回滾後，啟動器會清除 `just_updated` 旗標，防止連續回滾造成無窮迴圈。
